...
export default async function handler(req, res) {
  ...
  try {
    ...
    const subscribeUrl = `https://graph.facebook.com/v21.0/${pageId}/subscribed_apps?subscribed_fields=messages,messaging_postbacks,messaging_optins&access_token=${pageAccessToken}`;
    ...
    if (supabaseUrl && supabaseServiceKey) {
      ...
      // Get tenant_id from state or from the authenticated user's restaurant
      let tenantId = null;
      try {
      ...
      // If no tenant from state, find the first restaurant for this user
      if (!tenantId) {
        const { data: restaurants } = await supabase
          .from('restaurants')
          .select('id')
          .limit(1);
        tenantId = restaurants?.[0]?.id || null;
      }
      ...
      if (tenantId) {
        ...
        // Update restaurant with page/account IDs for webhook routing
        const updateData = channel === 'instagram'
