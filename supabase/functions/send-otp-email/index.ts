...
serve(async (req) => {
  ...
  try {
    const { to, otp, appName, subject } = await req.json()

    if (!to || !otp) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: to, otp' }),
    ...
    const emailSubject = subject || `${appName || 'OrderFlow'}: Din engangskode`
    const htmlContent = generateEmailHTML(otp, appName)
    const textContent = generateEmailText(otp, appName)
